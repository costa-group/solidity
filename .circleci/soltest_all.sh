...
[[ " $RUN_STEPS " == *" $STEP "* ]] && EVM=berlin OPTIMIZE=1 ABI_ENCODER_V1=1 BOOST_TEST_ARGS="-t !smtCheckerTests" "${REPODIR}/.circleci/soltest.sh"
STEP=$((STEP + 1))
...
do
    ...
    do
        ...
        ENFORCE_GAS_ARGS=""
        [ "${EVM}" = "berlin" ] && ENFORCE_GAS_ARGS="--enforce-gas-cost"
